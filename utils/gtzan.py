...
# Revised from torchaudio gtzan.py
...
def load_gtzan_rhythm(field, path):
    assert os.path.isdir(
        os.path.join(path, '..' ,'gtzan_tempo_beat')
    )
    beat_fileid = 'gtzan_' + field.replace('.', '_') + '.beats'
    beat_file_path = os.path.join(path, '..', 'gtzan_tempo_beat', 'beats', beat_fileid)
    ts = pd.read_csv(beat_file_path, delimiter='\t', header=None, names=['Time', 'Identifier'])['Time'].tolist()
    return ts 
...
class GTZAN(Dataset):
    ...
    def __init__(
        ...
        root: Union[str, Path],
        url: str = URL,
        folder_in_archive: str = FOLDER_IN_ARCHIVE, # raw
        download: bool = False,
